...
const API_URL = `${API_BASE_URL}/navysizes/`;
const NAVY_TYPES_URL = `${API_BASE_URL}/navytypes/`;
...
const NavySizeDashboard = () => {
  ...
  // Edit states
  const [editingSize, setEditingSize] = useState<NavySize | null>(null);
  ...
  const fetchTypes = async () => {
    try {
      const response = await fetch(NAVY_TYPES_URL, {
        headers: {
          'Authorization': `Bearer ${TOKEN}`,
          'Content-Type': 'application/json'
        },
      ...
      console.error("خطا در دریافت انواع", err);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert("✅ سایز جدید با موفقیت اضافه شد");
      setName("");
      setLogo(null);
      setSelectedTypes([]);
      fetchSizes();
  ...
  const handleEdit = (size: NavySize) => {
    setEditingSize(size);
    setEditName(size.name);
    setEditLogo(null);
    setEditSelectedTypes(size.types.map(type => type.id));
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    editSelectedTypes.forEach(typeId => {
      formData.append("type_ids", typeId.toString());
    });

    try {
      const response = await fetch(`${API_URL}${editingSize.id}/`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${TOKEN}`,
        },
      ...
      alert("✅ سایز با موفقیت بروزرسانی شد");
  ...
  const handleEditTypeToggle = (typeId: number) => {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">مدیریت سایز ناوگان</h1>
          <p className="text-gray-600">افزودن و مدیریت سایز‌های مختلف ناوگان دریایی</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  placeholder="مثال: ون ، کشنده ، کامیونت..."
                  value={name}
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100">
          <div className="p-6 border-b border-gray-100">
            <h2 className="text-2xl font-semibold text-gray-800 flex items-center">
              ...
              لیست سایز‌های ناوگان
            </h2>
            <p className="text-gray-600 mt-1">مدیریت سایز‌های موجود در سیستم</p>
          </div>
          ...
          {sizes.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">هیچ سایز‌ای یافت نشد</h3>
              <p className="text-gray-500">برای شروع، یک سایز جدید اضافه کنید</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    <div className="flex-shrink-0">
                      {size.logo ? (
                        ...
                        <div className="w-16 h-16 bg-gradient-to-br from-green-100 to-green-200 rounded-lg flex items-center justify-center border-2 border-gray-200">
                    ...
                    <div>
                      ...
                      <p className="text-gray-500">شناسه: {size.id}</p>
                      {size.types.length > 0 && (
                        <div className="mt-2">
                          <p className="text-sm text-gray-600 mb-1">انواع مرتبط:</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-gray-600 mt-1">ویرایش اطلاعات سایز: {editingSize.name}</p>
            </div>
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-6">
              <div>
                ...
                <input
                  type="text"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                  placeholder="نام جدید سایز ناوگان"
                  value={editName}
              ...
              <div>
                ...
                {editingSize.logo && (
                  <div className="mt-3">
                    ...
                    <img
                      src={`${editingSize.logo}`}
                      alt={editingSize.name}
                  ...
                  {types.map((type) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() => handleEditTypeToggle(type.id)}
                      />
                      <span className="text-sm font-medium">{type.name}</span>
                    </label>
                  ))}
...
export default NavySizeDashboard;
