...
const API_URL = `${API_BASE_URL}/navymehvars/`;
...
const NavyBrandDashboard = () => {
  ...
  const [mehvars, setMehvars] = useState<NavyMehvar[]>([]);
  ...
  // Edit states
  const [editingmehvar, setEditingMehvar] = useState<NavyMehvar | null>(null);
  const [editName, setEditName] = useState("");
  const [editLogo, setEditLogo] = useState<File | null>(null);
  const [editSelectedSizes, setEditSelectedSizes] = useState<number[]>([]);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);

  const fetchMehvars = async () => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert("✅ محور جدید با موفقیت اضافه شد");
      setName("");
      setLogo(null);
      setSelectedSizes([]);
      fetchMehvars();
  ...
  const handleEdit = (brand: NavyMehvar) => {
    setEditingMehvar(brand);
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`${API_URL}${editingmehvar.id}/`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${TOKEN}`,
        },
      ...
      alert("✅ محور با موفقیت بروزرسانی شد");
      setIsEditModalOpen(false);
      setEditingMehvar(null);
      setEditName("");
      setEditLogo(null);
  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">مدیریت محور ناوگان</h1>
          <p className="text-gray-600">افزودن و مدیریت محورهای مختلف ناوگان دریایی</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100">
          <div className="p-6 border-b border-gray-100">
            ...
            <p className="text-gray-600 mt-1">مدیریت محورهای موجود در سیستم</p>
          </div>
          ...
          {mehvars.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">هیچ محوری یافت نشد</h3>
              <p className="text-gray-500">برای شروع، یک محور جدید اضافه کنید</p>
            </div>
          ) : (
              ...
              {mehvars.map((mehvar) => (
                ...
                >
                  <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    <div>
                      <h3 className="text-xl font-semibold text-gray-900">{mehvar.name}</h3>
                      <p className="text-gray-500">شناسه: {mehvar.id}</p>
                      {mehvar.sizes.length > 0 && (
                        <div className="mt-2">
                          <p className="text-sm text-gray-600 mb-1">سایزهای مرتبط:</p>
                          <div className="flex flex-wrap gap-1">
                            {mehvar.sizes.map((size) => (
                              <span
                                key={size.id}
                  ...
                  <div className="flex space-x-2 rtl:space-x-reverse opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    ...
                    <button
                      onClick={() => handleDelete(mehvar.id)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100">
              ...
              <p className="text-gray-600 mt-1">ویرایش اطلاعات محور: {editingmehvar.name}</p>
            </div>
            ...
            <form onSubmit={handleUpdate} className="p-6 space-y-6">
              <div>
                ...
                <input
                  type="text"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-200"
                  placeholder="نام جدید محور ناوگان"
                  value={editName}
              ...
              <div>
                ...
                {editingmehvar.logo && (
                  <div className="mt-3">
                    ...
                    <img
                      src={`${editingmehvar.logo}`}
                      alt={editingmehvar.name}
